...
class Sheep {
    ...
    graze() {
        // console.log("graze")
        // console.log(this.grazing)

        // default home if no food
        if (!this.nearestFood) {
            this.wonder()
            // this.seek(createVector(width/2,height/2))
        }

        else if (this.nearestFoodDistance > this.maxReach) {
            // this.grazing = true
            this.seek(this.nearestFoodLocation)
        } else if (field[this.nearestFood].growth > this.maxC + this.cRate) {
            this.grazing = true
    ...
    herd() {
        if (this.neighbors.length > 0) {
            this.seek(this.neighborsCentroid)
        } else if (this.neighbors == 0){
            // console.log(this.neighborsScent)
            this.seek(this.nearestNeighborScentLocation)
        }
